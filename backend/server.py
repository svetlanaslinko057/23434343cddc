...
async def initiate_deliverable_payment(
    ...
    # Generate mock payment URL (replace with real WayForPay in production)
    payment_id = f"pay_{uuid.uuid4().hex[:12]}"
    mock_payment_url = f"/api/payments/wayforpay/mock-page?payment_id={payment_id}&invoice_id={invoice_id}"
    ...
    await db.invoices.update_one(
        {"invoice_id": invoice_id},
        {"$set": {"payment_url": mock_payment_url, "payment_provider": "wayforpay_mock"}}
    )
...
async def create_wayforpay_payment(
    ...
    payment_id = f"pay_{uuid.uuid4().hex[:12]}"
    mock_payment_url = f"/api/payments/wayforpay/mock-page?payment_id={payment_id}&invoice_id={request.invoice_id}"
    ...
    await db.invoices.update_one(
        ...
        {"$set": {
            "status": "pending_payment",
            "payment_provider": "wayforpay_mock",
            "payment_url": mock_payment_url,
            "payment_id": payment_id
        }}
    )
    
    logger.info(f"PAYMENT CREATED: {payment_id} for invoice {request.invoice_id}")
    return {
        "payment_id": payment_id,
        "payment_url": mock_payment_url,
...
async def wayforpay_callback(callback: PaymentCallbackRequest):
    """Handle WayForPay payment callback"""
    invoice = await db.invoices.find_one({"invoice_id": callback.invoice_id}, {"_id": 0})
    if not invoice:
        raise HTTPException(status_code=404, detail="Invoice not found")
    
    payment_status = "paid" if callback.status == "approved" else "failed"
    
    # Update payment record
    await db.payments.update_one(
        {"invoice_id": callback.invoice_id, "status": "pending"},
        {"$set": {
    ...
    if callback.status == "approved":
        ...
        # UNLOCK DELIVERABLE if linked
        deliverable_id = invoice.get("deliverable_id")
        if deliverable_id:
            await db.deliverables.update_one(
                {"deliverable_id": deliverable_id},
                {"$set": {
            ...
            logger.info(f"DELIVERABLE UNLOCKED: {deliverable_id}")
        
        # UPDATE PROJECT - allow next stage
        project_id = invoice.get("project_id")
        if project_id:
            await db.projects.update_one(
                {"project_id": project_id},
                {"$set": {"payment_status": "current", "last_payment_at": datetime.now(timezone.utc).isoformat()}}
            )
        
        # EMIT REALTIME EVENT
        await realtime.emit_to_project(invoice.get("project_id", ""), "payment.completed", {
        ...
        try:
            paid_invoice = await db.invoices.find_one({"invoice_id": callback.invoice_id}, {"_id": 0})
            if paid_invoice:
                await process_referral_payout(paid_invoice)
        except Exception as e:
    ...
    logger.info(f"PAYMENT CALLBACK: {callback.invoice_id} -> {payment_status}")
...
async def client_pay_deliverable(deliverable_id: str, user: User = Depends(get_current_user)):
    ...
    # Generate WayForPay mock payment URL
    backend_url = os.environ.get("BACKEND_URL", "")
    payment_url = f"{backend_url}/api/payments/wayforpay/mock-page?payment_id={payment_id}&invoice_id={invoice_id}"
    
    # Update invoice with payment URL
    await db.invoices.update_one(
        {"invoice_id": invoice_id},
        {"$set": {"payment_url": payment_url, "payment_provider": "wayforpay"}}
    )
    ...
    logger.info(f"PAYMENT INITIATED: deliverable={deliverable_id}, invoice={invoice_id}, amount=${price}")
