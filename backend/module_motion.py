...
async def _advance_project(db, project: Dict[str, Any]) -> None:
    ...
    for mod in sorted(by_status.get("review", []), key=lambda x: x.get("review_at") or ""):
        ...
        # If this module had an assignee, credit them.
        ...
        if assignee:
            ...
            if price > 0:
                await db.payouts.insert_one({
                    ...
                    "amount": round(price * 0.6, 2),  # dev share; rest is platform/qa
                    "status": "approved",
                    "created_at": now.isoformat(),
                })
            # EVENT BRIDGE: dev узнаёт что модуль закрыт и он заработал.
            dev_share = round(float(mod.get("final_price") or mod.get("price") or 0) * 0.6, 2)
