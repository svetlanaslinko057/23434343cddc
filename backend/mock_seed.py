...
async def seed_extra_client_demo(db) -> None:
    ...
    # Persist modules — but build payouts BEFORE stripping side-fields,
    # so each module's earned_amt / payout_status survives.
    payouts: List[Dict[str, Any]] = []
    for src in (p1_mods + p2_mods):
        st = src.get("payout_status")
        amt = src.get("earned_amt") or 0
        if not st or amt <= 0:
            continue
        po: Dict[str, Any] = {
            "payout_id": _new_id("po"),
        ...
        if st == "paid":
            po["paid_at"] = _iso(now - timedelta(days=1))
        payouts.append(po)
    ...
    logger.info(
        "MOCK SEED: 2 projects, %d modules, %d payouts, %d invoices, "
        "%d deliverables, %d tickets, %d notifications planted for %s",
        len(modules), len(payouts), len(invoices),
