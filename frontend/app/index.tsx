...
export default function Index() {
  ...
  const onPickFile = () => {
    if (Platform.OS !== 'web') {
      setError('File attachments are available on the web preview.');
      return;
    }
  ...
  const onFileChosen = async (ev: any) => {
    ...
    if (!isText) {
      setError(`Only .txt / .md files can be read in the browser. PDF/DOC coming soon — paste the key parts for now.`);
      ev.target.value = '';
      return;
    }
    if (file.size > MAX_FILE_BYTES) {
      setError(`File too large (${Math.round(file.size / 1024)}KB). Max 400KB.`);
      ev.target.value = '';
      return;
    }
    try {
      const text = await file.text();
      setAttachment({ name: file.name, text: text.slice(0, 5000) });
      setError('');
    } catch (e: any) {
    ...
    <KeyboardAvoidingView style={s.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      >
        ...
        <View style={s.attachRow}>
          <TouchableOpacity testID="visitor-attach-btn" style={s.attachBtn} onPress={onPickFile}>
            <Ionicons name="attach" size={16} color={T.textMuted} />
            <Text style={s.attachText}>
              {attachment ? `Attached: ${attachment.name}` : 'Attach brief (.txt / .md)'}
            </Text>
          </TouchableOpacity>
          {attachment && (
        ...
        {MODES.map((m) => {
          ...
          return (
            ...
            >
              {m.popular && (
                <View style={[s.popularBadge, { backgroundColor: m.color + '22', borderColor: m.color + '88' }]}>
                  <Text style={[s.popularBadgeText, { color: m.color }]}>RECOMMENDED</Text>
                </View>
              )}
...
const s = StyleSheet.create({
  ...
  popularBadgeText: { fontSize: 9, fontWeight: '800', letterSpacing: 1.5 },
