...
export default function AuthScreen() {
  ...
  const afterAuth = async () => {
    ...
    if (isDeveloperIntent) { try { await api.post('/developer/apply'); } catch { /* non-fatal */ } }
    try {
  ...
  const tryDemo = async () => {
    ...
    try {
      ...
      setError(extractErrorMessage(e, 'Demo failed'));
    } finally { setBusy(false); }
  };
          ...
          {step === 'code' && (
            <>
              ...
              {/* DEV banner — only shown when backend is in DEV mode
                  (no email delivery wired yet) */}
              {devCode && (
                <View style={s.devBanner} testID="auth-dev-code-banner">
